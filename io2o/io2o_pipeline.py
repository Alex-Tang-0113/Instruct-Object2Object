...
from io2o.io2o_datamanager import (
    InstructObject2ObjectDataManagerConfig,
)
from io2o.ip2p import InstructPix2Pix


@dataclass
class InstructObject2ObjectPipelineConfig(VanillaPipelineConfig):
    """Configuration for pipeline instantiation"""

    _target: Type = field(default_factory=lambda: InstructNeRF2NeRFPipeline)
    """target class to instantiate"""
    datamanager: InstructObject2ObjectDataManagerConfig = InstructObject2ObjectDataManagerConfig()
    """specifies the datamanager config"""
    prompt: str = "don't change the image"
    """prompt for InstructPix2Pix"""
    prompt_scene: str = "don't change the image"
    """prompt for InstructPix2Pix"""
    start_editting: int = 25000
...
class InstructNeRF2NeRFPipeline(VanillaPipeline):
    ...
    config: InstructObject2ObjectPipelineConfig
    ...
    def __init__(
        ...
        self.text_embedding_scene = self.ip2p.pipe._encode_prompt(
            self.config.prompt_scene, device=self.ip2p_device, num_images_per_prompt=1, do_classifier_free_guidance=True, negative_prompt=""
        )
    ...
    def prompt_callback(self, handle: ViewerText) -> None:
        ...
        '''currently not implemented for scene editing'''
    ...
    def get_train_loss_dict(self, step: int):
        ...
        if (step > self.config.start_editting and step % self.config.edit_rate == 0):
            ...
            for i in range(self.config.edit_count):
                ...
                original_image = self.datamanager.original_image_batch["image_object"][current_spot].to(self.device)
                original_image_scene = self.datamanager.original_image_batch["image_scene"][current_spot].to(self.device)
                ...
                original_image_scene = original_image_scene.unsqueeze(dim=0).permute(0, 3, 1, 2)
                ...
                rendered_image_scene = camera_outputs["rgb_scene"].unsqueeze(dim=0).permute(0, 3, 1, 2)
                ...
                edited_image_scene = self.ip2p.edit_image(
                            self.text_embedding_scene.to(self.ip2p_device),
                            rendered_image_scene.to(self.ip2p_device),
                            original_image_scene.to(self.ip2p_device),
                            guidance_scale=self.config.guidance_scale*4,
                            image_guidance_scale=self.config.image_guidance_scale,
                            diffusion_steps=self.config.diffusion_steps,
                            lower_bound=self.config.lower_bound,
                            upper_bound=self.config.upper_bound,
                ...
                if (edited_image_scene.size() != rendered_image_scene.size()):
                    edited_image_scene = torch.nn.functional.interpolate(edited_image_scene, size=rendered_image_scene.size()[2:], mode='bilinear')

                # write edited image to dataloader
                self.datamanager.image_batch["image_object"][current_spot] = edited_image.squeeze().permute(1,2,0)
                self.datamanager.image_batch["image_scene"][current_spot] = edited_image_scene.squeeze().permute(1,2,0)
