...
@dataclass
class InstructObject2ObjectTrainerConfig(TrainerConfig):
    """Configuration for the InstructNeRF2NeRFTrainer."""
    _target: Type = field(default_factory=lambda: InstructObject2ObjectTrainer)


class InstructObject2ObjectTrainer(Trainer):
    """Trainer for InstructObject2Object"""
    ...
    def reset_callback(self, handle: ViewerButton) -> None:
        ...
        # reset dataset
        self.config.pipeline.datamanager.image_batch['image_object'] = self.config.pipeline.datamanager.original_image_batch['image_object'].clone()
        self.config.pipeline.datamanager.image_batch['image_scene'] = self.config.pipeline.datamanager.original_image_batch['image_scene'].clone()
